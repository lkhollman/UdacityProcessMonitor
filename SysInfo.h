...
//is a series of setters and getters with the values that can be retrieved through the ProcessParser class
//and use items from util to transform the data into a suitable format
class SysInfo {
private:
...
public:
    ...
    System data is set
    */
        this->getOtherCores(ProcessParser::getNumberOfCores());  //added ProcessParser::
...
//This method updates and creates new datasets for CPU calculation. Every core is updated and previous data becomes the 
//current data of calculated measures.
...
// Getting data from files (previous data is required)
    ...
    // after acquirement of data we are calculating every core percentage of usage
        this->coresStats[i] = ProcessParser::printCpuStats(this->lastCpuCoresStats[i],this->currentCpuCoresStats[i]);  //printCpuStats was spelled PrintCpuStats. fixed
    }
...
// getting parsed data
    ...
    this->cpuPercent = ProcessParser::printCpuStats(this->lastCpuStats,this->currentCpuStats);  //printCpuStats was spelled PrintCpuStats. fixed
...
//This method creates a string with a progress bar. The bar shows the current status of aggregate CPU utilization, or the 
//utilization of a selected core.
...
//Wrappers
//Some methods are wrappers around Process Parser method. They are:
